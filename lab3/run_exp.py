...
exp_data_file = f'report/data/experiment.csv'
...
if run_tsp:
    ...
    for comb in random.sample(combinations, 10):
        for file_path in files:
            cmd = (
                f"./tsp -f {file_path} -s {exp_data_file} -e"
                f" -mi {comb['max-iter']}"
                f" -ps {comb['population-size']}"
                f" {'-spc' if comb['sp-crossover'] else ''}"
                f" -mp {comb['mutation-prob']}"
            )
            print(cmd)
...
output_df = pd.read_csv(exp_data_file)
