...
params = {
    'init_temperature': {10., 50., 100.},
    'alpha': {0.9, 0.95, 0.99},
    'min_temperature': {0.01, 0.1, 1.0},
    'max_iter': {0.1, 0.2, 0.3}
}
...
if run_tsp:
    ...
    for comb in random.sample(combinations, 50):
        for file_path in files:
            cmd = (
                f"./tsp -f {file_path} -m {tsp_mode} -s {data_file} -e"
                f" -sit {comb['init_temperature']}"
                f" -sa {comb['alpha']}"
                f" -smt {comb['min_temperature']}"
                f" -smi {comb['max_iter']}"
            )
            print(cmd)
